...
function Toggle({ on, onToggle }: { on: boolean; onToggle: () => void }) {
  return (
    ...
    >
      <div
        className={`absolute top-1 ${on ? "right-1" : "left-1"} w-4 h-4 bg-white rounded-full transition-all shadow-sm`}
      />
    </div>
  );
}
...
function SetRow({
  icon, iconBg, iconColor, name, sub, onClick, right, danger,
}: {
  ...
  return (
    <div
      onClick={onClick}
      className={`flex items-center justify-between py-3 md:py-[14px] px-4 md:px-5 border-b border-border last:border-b-0 transition-colors ${
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className={`text-[12px] md:text-[13px] ${danger ? "text-red" : "text-text"}`}>{name}</div>
...
function ProPlanCard({ label, desc, activeLabel }: { label: string; desc: string; activeLabel: string }) {
  return (
    <div
      className="rounded-xl md:rounded-lg p-4 md:p-5 mb-5 relative overflow-hidden"
      style={{
        background: "linear-gradient(135deg, #fdf9f0 0%, #f9f1e0 50%, #fdf9f0 100%)",
        border: "1px solid rgba(180,140,30,0.35)",
        boxShadow: "0 4px 24px rgba(180,140,30,0.1), inset 0 1px 0 rgba(255,220,80,0.4)",
      }}
    >
      ...
      <div className="flex items-start justify-between relative">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" style={{ color: "#a07820" }}>
              <path d="M2 20h20M5 20L3 8l5 4 4-7 4 7 5-4-2 12" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            <h3
              className="font-fraunces text-[15px] md:text-[18px] font-light tracking-wide"
              style={{ color: "#6b4e12" }}
            >
              {label}
            </h3>
          </div>
          <p className="text-[11px] md:text-[12px]" style={{ color: "rgba(120,90,20,0.55)" }}>
            {desc}
          </p>
        </div>
        <div
          className="text-[10px] md:text-[11px] font-medium py-1 px-2.5 md:px-3 rounded-md font-dm-mono mt-0.5"
          style={{
...
export default function SettingsClient({
  ...
  const DEFAULT_CHECKLIST = [
    ...(t.settings?.defaultChecklistItems ?? [
    ...
    const checkAndNotify = () => {
      ...
      if (now.getHours() === h && now.getMinutes() === m) {
        if (typeof Notification !== "undefined" && Notification.permission === "granted") {
          new Notification("TradeLog", { body: s.notificationBody, icon: "/favicon.ico" });
        }
      }
    };
  ...
  async function saveAccount() {
    ...
    if (!error) {
      setProfile((p) =>
        p ? { ...p, account_balance: parseFloat(balance), default_risk: parseFloat(riskPct), currency, strategy } : p
      );
      setModal(null);
  ...
  return (
    <div>
      ...
      {/* Plan card */}
      {isPro ? (
        <ProPlanCard label={s.proPlan} desc={s.proDesc} activeLabel={s.active} />
      ) : (
        <div className="bg-teal-bg border border-teal-br rounded-xl md:rounded-lg p-4 md:p-5 mb-5 flex items-center justify-between">
          <div>
            <h3 className="font-fraunces text-[15px] md:text-[18px] font-light text-text">{s.freePlan}</h3>
            <p className="text-[11px] md:text-[12px] text-text-2 mt-1">{s.freeDesc}</p>
          </div>
      ...
      {/* Account section */}
      <div className="text-[10px] font-medium tracking-[0.12em] uppercase text-text-3 mb-2 mt-4 px-1">
        {s.sectionAccount}
      </div>
        ...
        <SetRow
          ...
          sub={
            profile?.account_balance
              ? `$${profile.account_balance} · ${profile.default_risk}% risk${profile.strategy?.length ? ` · ${profile.strategy.join(", ")}` : ""}`
              : s.notEntered
          }
        ...
        <SetRow
          icon={
            <>
              ...
              <path
                d="M12 2v3M12 19v3M2 12h3M19 12h3M4.93 4.93l2.12 2.12M16.95 16.95l2.12 2.12M16.95 7.05l2.12-2.12M4.93 19.07l2.12-2.12"
                stroke="currentColor"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
      ...
      {modal && (
        <div
          className="fixed inset-0 bg-[rgba(0,0,0,0.5)] backdrop-blur-sm z-50 flex items-center justify-center p-4 md:p-6"
          onClick={(e) => {
            if (e.target === e.currentTarget) setModal(null);
          }}
        >
          <div className="bg-surface border border-border rounded-2xl p-5 md:p-7 w-full max-w-[600px] max-h-[80vh] overflow-y-auto shadow-[0_20px_60px_rgba(0,0,0,0.2)]">
            {modal === "account" && (
              <>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {STRATEGIES.map((strat) => (
                      <button
                        ...
                        onClick={() =>
                          setStrategy((p) =>
                            p.includes(strat) ? p.filter((x) => x !== strat) : [...p, strat]
                          )
                        }
                        className={`py-1.5 md:py-[6px] px-2.5 md:px-3 rounded-lg text-[11px] md:text-[12px] font-medium cursor-pointer transition-all border ${
                          strategy.includes(strat)
                ...
                <button
                  onClick={saveAccount}
                  className="w-full py-3 bg-text text-bg border-none rounded-lg font-dm-sans text-[13px] md:text-[14px] font-medium cursor-pointer mt-4"
                >
                  {s.save}
                </button>
                <button
              ...
              <>
                ...
                {!showCheckout ? (
                  <>
                    {/* Free plan card */}
                    <div
                      className={`border rounded-lg p-3 md:p-4 mb-2.5 ${
                        !isPro ? "bg-teal-bg border-teal-br" : "bg-surface2 border-border"
                      }`}
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <div className="font-medium text-[13px] md:text-[14px] text-text">{s.freePlan}</div>
                        {!isPro && (
                          <span className="bg-teal text-white text-[9px] font-medium py-0.5 px-2 rounded font-dm-mono">
                            {s.active}
                          </span>
                        )}
                      </div>
                      <div className="text-[15px] md:text-[16px] font-dm-mono font-medium text-text-2">
                        $0<span className="text-[12px] font-normal text-text-3">{s.perMonth}</span>
                      </div>
                      <div className="text-[11px] md:text-[12px] text-text-3 mt-0.5">{s.freeDesc}</div>
                    </div>

                    {/* Pro plan card */}
                    <div
                      className={`border rounded-lg p-3 md:p-4 mb-2.5 transition-opacity ${
                        isPro
                          ? "bg-teal-bg border-teal-br"
                          : "bg-surface2 border-border cursor-pointer hover:opacity-80"
                      }`}
                      onClick={!isPro ? () => setShowCheckout(true) : undefined}
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-[15px] md:text-[16px] text-teal font-dm-mono font-medium">
                            $14<span className="text-[12px] font-normal text-text-3">{s.perMonth}</span>
                          </div>
                          <div className="text-[11px] md:text-[12px] text-text-3 mt-0.5">{s.proFeaturesDesc}</div>
                        </div>
                        {!isPro && <span className="text-text-3 text-[18px] ml-2">›</span>}
                      </div>
                    </div>
                  </>
                ) : (
              ...
              <>
                ...
                <div className="flex gap-2 mt-3.5">
                  <input
                    ...
                    placeholder={s.checklistPlaceholder}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        if (newItem.trim()) {
                  ...
                  <button
                    ...
                    className="py-2 md:py-2.5 px-3.5 bg-text text-bg border-none rounded-lg cursor-pointer font-medium text-[12px] md:text-[13px] whitespace-nowrap"
                  >
                    {s.add}
                  </button>
                </div>
              ...
              <>
                ...
                <div className="flex flex-wrap gap-1.5 mb-3.5">
                  {confluenceTags.map((tag, i) => (
                    <span
                      key={i}
                      className="inline-flex items-center gap-1.5 py-1 md:py-[5px] px-2.5 md:px-3 rounded-lg text-[11px] md:text-[12px] bg-purple-bg text-purple border border-purple-br"
                    >
                      {tag}
                      <button
                        onClick={() => setConfluenceTags((p) => p.filter((_, idx) => idx !== i))}
                        className="bg-none border-none text-red cursor-pointer text-[12px] md:text-[14px] px-0 leading-[1]"
                      >
                        ×
                      </button>
                    </span>
                ...
                <div className="flex gap-2">
                  <input
                    value={newItem}
                    onChange={(e) => setNewItem(e.target.value)}
                    placeholder={s.confluencePlaceholder}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        if (newItem.trim()) {
                          setConfluenceTags((p) => [...p, newItem.trim()]);
                          setNewItem("");
                        }
                      }
                    }}
            ...
            {modal === "clear" && (
              <>
                ...
                <p className="text-[12px] md:text-[13px] text-text-2 leading-[1.6] mb-6">{s.clearDesc}</p>
                <button
                  onClick={clearAllData}
                  className="w-full py-3 bg-red text-white border-none rounded-lg font-dm-sans text-[13px] md:text-[14px] font-medium cursor-pointer mb-2"
                >
                  {s.clearConfirm}
                </button>
                <button
                  onClick={() => setModal(null)}
                  className="w-full py-2.5 bg-none text-text-2 border-none text-[12px] md:text-[13px] cursor-pointer"
                >
                  {s.cancel}
                </button>
              </>
