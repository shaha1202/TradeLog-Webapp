...
function LockedWidget({ title, desc, upgradeToPro }: { title: string; desc: string; upgradeToPro: string }) {
  return (
    <div className="relative rounded-2xl border border-border overflow-hidden mb-5 shadow-[var(--shadow)]">
      {/* Fake blurred content */}
      <div className="blur-sm select-none pointer-events-none p-5 bg-surface">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-9 h-9 rounded-lg bg-surface2" />
          <div>
            <div className="h-3 bg-surface2 rounded w-24 mb-1.5" />
            <div className="h-2.5 bg-surface2 rounded w-16" />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div className="h-20 bg-surface2 rounded-lg" />
          <div className="h-20 bg-surface2 rounded-lg" />
          <div className="col-span-2 h-14 bg-surface2 rounded-lg" />
        </div>
      </div>
      {/* Overlay */}
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2.5 bg-surface/80 backdrop-blur-[3px]">
        <span className="text-[10px] font-dm-mono bg-teal text-white px-2.5 py-0.5 rounded-full leading-none">Pro</span>
        <p className="text-sm font-medium text-text text-center px-8 leading-snug">{title}</p>
        <p className="text-xs text-text-3 text-center px-10 leading-relaxed">{desc}</p>
        <Link
          href="/settings"
          className="mt-1 text-xs bg-teal text-white px-4 py-2 rounded-lg font-dm-sans font-medium hover:opacity-90 transition-opacity"
        >
          {upgradeToPro} →
        </Link>
      </div>
...
export default function StatsClient({ trades, profile }: { trades: Trade[]; profile: Profile | null }) {
  ...
  const effectivePeriod = isPro ? period : 7;

  const filtered = useMemo(() => filterByDays(trades, effectivePeriod), [trades, effectivePeriod]);
  ...
  return (
    <div>
      ...
      <div className="flex gap-1.5 md:gap-2 mb-5 overflow-x-auto pb-1">
        {[
          ...
          return (
            <button
              key={days}
              onClick={() => { if (!locked) setPeriod(days); }}
              disabled={locked}
              className={`relative px-3 md:px-4 py-1.5 md:py-2 rounded-lg text-[11px] md:text-[12px] font-medium transition-all whitespace-nowrap border ${
                !locked && period === days
                  ? "bg-text text-bg border-text"
                  : locked
                  ? "bg-surface text-text-3 border-border opacity-50 cursor-not-allowed"
                  : "bg-surface text-text-2 border-border cursor-pointer"
              }`}
            >
              {label}
              {locked && (
                <span className="ml-1.5 text-[8px] font-dm-mono bg-teal/20 text-teal px-1 py-0.5 rounded-full">
                  Pro
                </span>
              )}
            </button>
      ...
      {/* Free plan note */}
      {!isPro && (
        <div className="flex items-center gap-2 bg-amber-bg border border-amber-br rounded-xl px-4 py-3 mb-5 text-xs text-amber font-dm-sans">
          ...
          {filtered.length === 0 ? (
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-3 mb-5">
                {[
                  { label: st.totalPnl, value: formatPnl(stats.totalPnl, currency), color: stats.totalPnl >= 0 ? "g" : "r" },
                  { label: st.winRate, value: `${stats.winRate}%`, color: stats.winRate >= 50 ? "g" : stats.winRate >= 40 ? "" : "r" },
                  ...
                  <div key={label} className="bg-surface border border-border rounded-xl p-3 md:p-4 shadow-[var(--shadow)]">
                    ...
                    <div className={`text-[14px] md:text-[18px] font-medium font-dm-mono tracking-[-0.5px] ${color === "g" ? "text-green" : color === "r" ? "text-red" : "text-text"}`}>
                      {value}
                    </div>
                  </div>
                ))}
              ...
              {/* Balance curve locked */}
              <LockedWidget
                title={st.deepAnalytics}
                desc={st.freePeriodNote}
                upgradeToPro={st.upgradeToPro}
              />
            </>
          )}
            ...
            {filtered.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 mt-1">
                ...
                <div className="col-span-1 md:col-span-2 bg-surface2 border border-border rounded-lg p-3 md:p-4 flex gap-3 items-start">
                  ...
                  <div>
                    <div className="text-[10px] font-semibold tracking-[0.1em] uppercase text-text-3 mb-1">{st.aiKeyInsight}</div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                <div className="bg-surface border border-border rounded-2xl p-4 md:p-6 shadow-[var(--shadow)]">
                  <div className="text-[10px] font-medium tracking-[0.12em] uppercase text-text-3 mb-4">{st.bestWorst}</div>
                  {[
                    { label: st.bestTrade, trade: stats.best, color: "var(--green)" },
                    { label: st.worstTrade, trade: stats.worst, color: "var(--red)" },
                  ].map(({ label, trade: tr, color }) => (
                    <div key={label} className="bg-surface2 rounded-lg p-3 md:p-3.5 mb-2.5 last:mb-0">
                      ...
                      <div className="text-[13px] md:text-[15px] font-medium text-text">{tr?.asset ?? "—"}</div>
                      <div className="text-[13px] md:text-[14px] font-medium font-dm-mono mt-0.5" style={{ color }}>
                        {tr?.pnl !== undefined ? formatPnl(tr.pnl ?? null, currency) : "—"}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {moodPerf.length > 0 && (
                <div className="bg-surface border border-border rounded-2xl p-4 md:p-6 mb-5 shadow-[var(--shadow)]">
                  <div className="text-[10px] font-medium tracking-[0.12em] uppercase text-text-3 mb-1">{st.moodPnl}</div>
                  <p className="text-[11px] text-text-3 mb-5">{st.moodPnlSub}</p>
                  <div className="space-y-3">
                    {moodPerf.map((m) => {
                      const maxAbsPnl = Math.max(...moodPerf.map((x) => Math.abs(x.pnl)), 1);
                      const barWidth = Math.min((Math.abs(m.pnl) / maxAbsPnl) * 100, 100);
                      const isPos = m.pnl >= 0;
                      return (
                        <div key={m.mood}>
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-[12px] font-medium text-text">{m.mood}</span>
                            <div className="flex items-center gap-3">
                              <span className="text-[10px] text-text-3 font-dm-mono">{m.count} {st.tradeCount} · {m.winRate}%</span>
                              <span className="font-dm-mono text-[12px] font-medium min-w-[60px] text-right" style={{ color: isPos ? "var(--green)" : "var(--red)" }}>
