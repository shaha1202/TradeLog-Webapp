...
export default function MobileNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-surface border-t border-border md:hidden z-50">
      <div className="flex items-center justify-around pt-1 pb-2 px-1">
        {navItems.map((item) => {
          ...
          return (
            <a
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center justify-center gap-0.5 px-4 py-1.5 no-underline transition-colors duration-150 ${
                active ? "text-teal" : "text-text-3"
              }`}
            >
              {/* Active indicator dot */}
              <div className={`w-1 h-1 rounded-full mb-0.5 transition-opacity duration-150 ${active ? "bg-teal opacity-100" : "opacity-0"}`} />
              <div className="w-4 h-4">
                {item.icon}
              </div>
              <span className="text-[10px] font-medium font-dm-sans">{item.label}</span>
            </a>
          );
        })}
      </div>
